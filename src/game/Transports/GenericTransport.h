...
class GenericTransport : public GameObject
{
    public:
        ...
        // Whether the vessel is between stop frames. Only an MO transport has the concept: it
        // travels a path with docks, and walking aboard one that has not stopped drops the
        // passenger in the water, which is what callers use this to avoid. A LocalTransport
        // animates continuously and never docks, and boarding one is safe at any point in its
        // loop because the boarding offset is measured in the same frame it is re-applied in
        // (see LocalTransport::AddPassenger), so the base answer is "not moving" - i.e. do not
        // block boarding - rather than "unknown".
